...
class checkUser {
  ...
  static async isCashier(req, res, next) {
    const userFund = await con.query(user.searchUserById, [req.user.id]);
    if (userFund.rowCount === 0) return res.status(401).json({ status: 401, error: 'please login or signup' });
    if (userFund.rows[0].type === 'staff' && userFund.rows[0].isadmin === true) return res.status(401).json({ status: 401, error: 'only cashier is allowed to debit/credit account' });
  ...
  static async isOwner(req, res, next) {
    ...
    if (checkOwner.rowCount !== 0) {
      if (checkOwner.rows[0].owner !== req.user.id && getUser.rows[0].type !== 'staff') return res.status(401).json({ status: 200, message: 'access denied' });
    }
    next();
  }
  ...
  static async isAllowed(req, res, next) {
    ...
    if (getUser.rows[0].userid !== req.user.id && getUser2.rows[0].type !== 'staff') return res.status(401).json({ status: 200, message: 'access denied' });
